...
// Fetchers
export const createFetchers = (solanaNetwork: SolanaNetwork, bitcoinNetwork: BitcoinNetwork) => {
  const { binanceApi, hermesApi, aegleApi, aresApi } = createAxiosInstances(
    solanaNetwork,
    bitcoinNetwork,
  );

  const binanceFetcher = async <T>(url: string, schema: ZodType<T>): Promise<T> => {
    try {
  ...
  const hermesFetcher = async <T>(url: string, schema: ZodType<T>): Promise<T> => {
    try {
  ...
  const aegleFetcher = async <T>(url: string, schema: ZodType<T>): Promise<T> => {
    try {
  ...
  const aresFetcher = async <T>(url: string, schema: ZodType<T>): Promise<T> => {
    try {
