...
const getGridTemplateColumns = (columnSizes?: number[]) => {
  ...
  return columnSizes.map((size) => (typeof size === 'number' ? `${size}fr` : size)).join(' ');
};
...
const Table = ({
  ...
  const validatedColumnSizes = columnSizes ? checkGridIsValid(columnSizes) : undefined;
        ...
        className={classNames(
          {
            ...
            'w-full md:overflow-auto': tableType === 'stacked' && width === 'stretch',
      ...
      >
        <div
          className={classNames('inline-block', {
            ...
            'w-full': tableType === 'default' && width === 'stretch',
            'w-full min-w-full md:w-auto': tableType === 'stacked' && width === 'auto',
          ...
          <TableContext.Provider value={tableContext}>
            <div
              className={classNames(
                ...
                tableType === 'stacked' && 'md:bg-sys-color-background-light p-0 md:p-8',
          ...
          <div
            ...
            className={classNames(
              'z-1 text-sys-color-text-mute first:rounded-l-8 last:rounded-r-8 relative whitespace-nowrap px-12',
              {
                ...
                'last-of-type:justify-self-end': lastItemAlignEnd && !stickLastItem,
                // Cell Sizing
...
const TableBody = ({ children, className }: TableBodyProps) => {
  ...
  const { tableType, variant, stickFirstItem, stickLastItem } = React.useContext(TableContext);

  return (
    <div
      className={classNames(
        // Stacked Table Styling
        tableType === 'stacked' && variant === 'nested' && 'flex flex-col gap-y-24 md:gap-y-0',
        tableType === 'stacked' && variant === 'separated' && 'flex flex-col gap-y-16 md:gap-y-0',
    ...
    >
      {rows.map((row, index) => (
        <React.Fragment key={index}>
          ...
          {index < rows.length - 1 && (
            <Divider className={classNames(tableType === 'stacked' && 'hidden md:block')} />
          )}
        </React.Fragment>
      ))}
    </div>
    ...
    <TableContext.Provider value={rowContext}>
      <div
        ...
        className={classNames(
          ...
          // Table Type Styling
          tableType === 'default' && width === 'auto' && 'grid w-max min-w-full',
          ...
          tableType === 'stacked' && variant === 'nested' && width === 'auto' && 'md:w-max',
          ...
          tableType === 'stacked' && variant === 'separated' && width === 'auto' && 'md:w-max',
          className,
        )}
        style={{
      ...
      >
        ...
        {React.Children.map(children, (child, index) => {
          const isHiddenOnMobile = (child as React.ReactElement).props.hideOnMobile;
          return (
            <>
              ...
              {showDividerOnMobile && tableType === 'stacked' && !isHiddenOnMobile && (
                <>
  ...
  return (
    <div
      className={classNames(
        ...
        {
          // Cell Sizing
          'body-body2-medium py-12': cellSize === 'small' && tableType === 'default',
          'body-body2-medium py-16': cellSize === 'medium' && tableType === 'default',
          'body-body1-medium py-12': cellSize === 'medium/16px' && tableType === 'default',
          'body-body1-medium py-24': cellSize === 'large' && tableType === 'default',
          'body-body2-medium md:py-12': cellSize === 'small' && tableType === 'stacked',
          'body-body2-medium md:py-16': cellSize === 'medium' && tableType === 'stacked',
          'body-body1-medium md:py-12': cellSize === 'medium/16px' && tableType === 'stacked',
          'body-body1-medium md:py-24': cellSize === 'large' && tableType === 'stacked',
          ...
          'last:ml-auto last:w-max last:justify-self-end': lastItemAlignEnd && !stickLastItem,
    ...
    >
      {showLabelsOnMobile && !hideLabelOnMobile && tableType === 'stacked' && headers && (
      ...
      >
        ...
        {children}
        {rightIcon && !rightIconOnClick && !rightIconOnMouseEnter && !rightIconOnMouseLeave && (
          <Icon
            name={rightIcon}
            size={rightIconSize}
        ...
        {rightIcon && (rightIconOnClick || rightIconOnMouseEnter || rightIconOnMouseLeave) && (
          ...
          >
            ...
            <Icon
              name={rightIcon}
              size={rightIconSize}
              className={classNames('flex-shrink-0 transition', rightIconClassName)}
            />
          </button>
        )}
        {showLockIcon && (
          <Icon name="Lock" size={18} className="text-sys-color-text-mute flex-shrink-0" />
        )}
      </div>
    </div>
  );
