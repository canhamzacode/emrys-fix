...
const Modal = ({
  ...
  const computedWidth = windowWidth < 550 ? '100%' : width ? `${width}px` : 'auto';
          ...
          >
            ...
            <motion.div
              ...
              className={classNames(
                ...
                // Hidden Styles
                backdropType === 'hidden' && 'absolute bg-transparent backdrop-blur-none',
                // Override Header
            ...
            {/* Card */}
            <motion.div
              initial={animateFrom ? { opacity: 0, y: animateFrom === 'top' ? -20 : 20 } : {}}
...
const ModalHeader = ({ title, onClose, className, hideCloseButton }: ModalHeaderProps) => {
      ...
      {title && (
        <div
          className={classNames('flex w-full items-center justify-center', {
            ...
            // Annotation Styles
            'text-sys-color-text-secondary headline-headline6 pt-4': type === 'emphasized',
          })}
        >
          {title}
        </div>
...
const ModalActions = ({ children, annotation, className }: ModalActionsProps) => {
  return (
    <div
      className={classNames('flex w-full flex-col items-center justify-center gap-y-24', className)}
    >
      <div className="relative z-10 flex w-full items-center gap-x-8">{children}</div>
      {annotation && (
        <div className="text-sys-color-text-secondary flex items-center gap-x-8">
          ...
          <span className="text-sys-color-text-mute body-body1-medium">{annotation}</span>
        </div>
      )}
    </div>
  );
