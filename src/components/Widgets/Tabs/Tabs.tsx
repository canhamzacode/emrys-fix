...
export default function Tabs({
  ...
  return (
    <div
      className={`${className} ${
        isSubTab
          ? `${subTabLayoutStyle ? subTabLayoutStyle : 'border-shade-cardLight w-fit rounded-2xl border p-1'} ${styles.subTabs}`
          : styles.tabs
      }`}
    >
      {tabs.map((tab, index) => {
        ...
        return (
          <button
            ...
            disabled={tab.isDisabled || (!solanaWalletConnected && loginRequired)}
            onClick={() => onClick(tab.value)}
          >
            {tab.icon && <div className={styles.tabs__tab__icon}>{tab.icon}</div>}
            <span>{tab.label}</span>
