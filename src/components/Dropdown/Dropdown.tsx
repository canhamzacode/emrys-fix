...
const Dropdown = ({
  ...
  const [currentSelectedIndex, setCurrentSelectedIndex] = useState<number | number[] | null>(
    selectedIndex,
  );
  ...
  const initialSearchValue =
    selectedIndex !== null && type === 'multi-function'
      ? (children as React.ReactElement<DropdownOptionProps>[])?.[selectedIndex as number]?.props
          .label || ''
      : '';
    ...
    if (currentSelectedIndex !== null) {
      if (Array.isArray(currentSelectedIndex)) {
        const selectedLabels = currentSelectedIndex.map(
          (index) => (children as React.ReactElement<DropdownOptionProps>[])?.[index]?.props.label,
        );
        return selectedLabels.join(', ');
      }
      return (children as React.ReactElement<DropdownOptionProps>[])?.[currentSelectedIndex]?.props
        .label;
    }
    return label;
  };
  ...
  const hasNoResults =
    type === 'multi-function' && searchValue && React.Children.count(filteredChildren) === 0;
        ...
        {type === 'default' || type === 'filter' ? (
          ...
          >
            {type === 'filter' && (
              <Icon
                name="Filter"
                className={classNames('flex-shrink-0 duration-200 ease-in-out', {
                  // Selected options
            ...
            <div className="flex w-full max-w-full items-center truncate">{renderLabel()}</div>
            {type === 'default' && (
              <Icon
                name={
                  dropdownIcon === 'chevron'
...
const DropdownOption = ({
  ...
  const { size, selectedIndex, setSelectedIndex, setIsOpen, type, searchValue, setSearchValue } =
  ...
  return (
    <button
      ...
      className={classNames(className, 'rounded-8 peer flex w-full items-start', {
    ...
    >
      {type === 'filter' && <Checkbox checked={isSelected} disabled={disabled} />}
      {children}
    </button>
  );
};
