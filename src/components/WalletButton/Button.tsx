...
const getDisplayLabel = ({
  ...
  if (solanaWalletConnected && bitcoinWalletRequired && !bitcoinWalletConnected) {
...
export default function Button({
  ...
  const [prevBitcoinWalletConnected, setPrevBitcoinWalletConnected] =
    useState(bitcoinWalletConnected);
  ...
  const handleClick = () => {
    ...
    } else if (solanaWalletConnected && bitcoinWalletRequired && !bitcoinWalletConnected) {
      ...
      {!isLoading ? (
        <div className={styles.btn__content}>
          {icon && iconPosition === 'left' && (
            <span className={styles.btn__icon} style={{ width: iconsize, height: iconsize }}>
              {icon}
            </span>
          )}
          {displayLabel && (
            <span className={`${styles.btn__label} ${truncateText && 'max-w-[80%] truncate'}`}>
              {displayLabel}
            </span>
          )}
          {hoveredIcon && <span className={styles.btn__animated__icon}>{hoveredIcon}</span>}
