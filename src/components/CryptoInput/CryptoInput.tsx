...
export default function CryptoInput({
  ...
  const displayOptions = dropdownOptions?.filter(
    (option) => option.label !== currentOption?.label || option.type !== currentOption?.type,
  );
  ...
  return (
    ...
    >
      <div className="group relative flex w-[150px] flex-shrink-0 transition sm:w-auto">
        <div
          className={`${dropdownOptions ? 'hover:!shadow-[0px_4px_4px_rgba(199, 217, 254,0.5),inset_0px_2px_15px_rgba(139,138,158,0.5)] hover:!border-shade-mute/50 cursor-pointer' : ''} ${styles.input__group__crypto} ${dropdownOptions && showDropdown ? 'relative !z-50' : ''}`}
          onClick={() => {
        ...
        >
          ...
          {dropdownOptions && (
            <div className="!ml-auto flex-shrink-0">
              <Icon
                name="ChevronDown"
                className="text-shade-secondary group-hover:text-shade-primary transition"
                size={12}
              />
            </div>
          )}
      ...
      <div className={styles.input__group__input}>
        ...
        <Divider />
        <div className={`${errorMessage && '!text-[#FF4646]'} ${styles.input__group__input__fiat}`}>
