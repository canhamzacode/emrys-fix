...
import { Interaction, InteractionStatus, InteractionType, UTXOs } from '@/types/api';
...
export default function ConfirmDepositModal({
  ...
  const setIsGlobalLoaderOpen = useStore((state) => state.setIsGlobalLoaderOpen);
  ...
  const handleConfirmDeposit = async () => {
    ...
    const hotReserveBuckets =
      await zplClient.getHotReserveBucketsByBitcoinXOnlyPubkey(userXOnlyPublicKey);
    ...
    try {
      ...
      const transaction: Interaction = {
        ...
        interaction_id: zplClient.deriveInteraction(Buffer.from(txId, 'hex'), new BN(0)).toBase58(),
      ...
      await transactionRepo.addInteraction(bitcoinNetwork, solanaPubkey.toBase58(), transaction);
  ...
  return (
    ...
    >
      ...
      <div className="z-10 flex w-full flex-col gap-y-24 pt-24 sm:pt-0">
        ...
        <div className="flex w-full items-center justify-between">
          <div className="flex items-center gap-x-12">
            ...
            <span className="body-body1-medium text-sys-color-text-primary">Confirm Deposit</span>
          </div>
        </div>
        ...
        <div className="flex flex-col gap-y-12">
          ...
          <div className="body-body1-medium text-sys-color-text-primary flex flex-col gap-y-8">
            <div className="flex items-center justify-between">
              <span>Miner Fee</span>
              <span>
                {formatValue(new BigNumber(minerFee).dividedBy(10 ** BTC_DECIMALS), 6)} BTC
              </span>
            </div>
          </div>
        </div>
        ...
        <div className="rounded-12 bg-sys-color-background-card border-apollo-border-15 flex flex-col gap-y-4 border p-16">
          <div className="flex flex-col gap-y-8">
            <div className="flex items-center gap-x-8">
              <Icon name="Alert" size={18} className="text-sys-color-state-warning" />
              <span className="body-body1-medium text-sys-color-text-primary">Notice</span>
            </div>
      ...
      <ModalActions className="!pt-40">
        <Button type="secondary" size="large" className="w-full" label="Cancel" onClick={onClose} />
        <Button
          type="primary"
          size="large"
          className="w-full"
  ...
  return (
    ...
    >
      <div className="flex w-full flex-row justify-between gap-y-8 sm:w-auto sm:flex-col sm:justify-start">
        <span className="body-body2-medium text-sys-color-text-secondary">Lock</span>
        <div className="flex items-center gap-x-8">
          ...
          {assetFrom.isLocked && <Icon name="Lock" className="text-sys-color-text-secondary" />}
        </div>
      </div>
      ...
      <div className="flex w-full flex-row items-center justify-between gap-y-8 sm:w-auto sm:flex-col sm:items-start sm:justify-start">
        <span className="body-body2-medium text-sys-color-text-secondary">Mint</span>
        <div className="flex items-center gap-x-8">
          ...
          {assetTo.isLocked && <Icon name="Lock" className="text-sys-color-text-secondary" />}
        </div>
      </div>
    </div>
  );
