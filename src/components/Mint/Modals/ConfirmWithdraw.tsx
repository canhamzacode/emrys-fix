...
import { Modal, ModalActions, ModalHeader } from '@/components/ShadcnModal/Modal';
...
export default function ConfirmWithdraw({
  ...
  const [selectedWallet, setSelectedWallet] = useState(connectedWallets?.[0] ?? '');
  ...
  const handleWithdraw = () => {
    const action = async () => {
      ...
      const withdrawAmountBN = new BN(
        new BigNumber(withdrawAmount).multipliedBy(10 ** BTC_DECIMALS).toString(),
      );
      ...
      if (assetFrom.isLocked) {
        ...
        const sortedPositions = positions.toSorted((a, b) =>
          b.storedAmount.sub(b.frozenAmount).cmp(a.storedAmount.sub(a.frozenAmount)),
        );
        ...
        for (const position of sortedPositions) {
          ...
          const twoWayPegGuardianSetting = twoWayPegGuardianSettings.find(
            (setting) =>
              zplClient
                .deriveLiquidityManagementGuardianSettingAddress(new PublicKey(setting.address))
        ...
        const twoWayPegGuardiansWithQuota = await Promise.all(
          twoWayPegGuardianSettings.map(async (twoWayPegGuardianSetting) => {
            const totalSplTokenMinted = new BN(twoWayPegGuardianSetting.total_amount_pegged);

            const splTokenVaultAuthority = zplClient.deriveSplTokenVaultAuthorityAddress(
              new PublicKey(twoWayPegGuardianSetting.address),
            );
            ...
            try {
              ...
              const splTokenBalance = new BN(tokenAccountData.amount.toString());
        ...
        const sortedTwoWayPegGuardiansWithQuota = twoWayPegGuardiansWithQuota.toSorted((a, b) =>
          b.remainingStoreQuota.cmp(a.remainingStoreQuota),
        );

        let remainingAmount = withdrawAmountBN.clone();
        for (const twoWayPegGuardian of sortedTwoWayPegGuardiansWithQuota) {
          const amountToWithdraw = BN.min(twoWayPegGuardian.remainingStoreQuota, remainingAmount);
      ...
      <div className="z-10 flex w-full flex-col gap-y-24 pt-24 sm:pt-0">
        ...
        <div className="flex w-full items-center justify-between">
          <div className="flex items-center gap-x-12">
            <Icon name="Stake" size={18} className="text-apollo-brand-primary-blue" />
            <span className="body-body1-medium text-sys-color-text-primary">Confirm Withdraw</span>
          </div>
        </div>
        ...
        <div className="z-50 flex flex-col gap-y-8">
          <span className="body-body1-semibold text-sys-color-text-primary">Send to</span>
          <Dropdown
        ...
        <div className="flex flex-col gap-y-12">
          <span className="body-body1-semibold text-sys-color-text-primary">Transaction Fee</span>
          <div className="body-body1-medium text-sys-color-text-primary flex flex-col gap-y-8">
            <div className="flex items-center justify-between">
              ...
              >
                ...
                >
                  The Service Fee is deducted from the final received {cryptoType} amount by
                  Orpheus.
                </Tooltip>
              </div>
  ...
  return (
    ...
    >
      <div className="flex w-full flex-row justify-between gap-y-8 sm:w-auto sm:flex-col sm:justify-start">
        <span className="body-body2-medium text-sys-color-text-primary">Burn</span>
        <div className="flex items-center gap-x-8">
          ...
          {assetFrom.isLocked && <Icon name="Lock" className="text-sys-color-text-primary" />}
        </div>
      </div>
      ...
      <div className="flex w-full flex-row items-center justify-between gap-y-8 sm:w-auto sm:flex-col sm:items-start sm:justify-start">
        <span className="body-body2-medium text-sys-color-text-primary">Unlock</span>
        <div className="flex items-center gap-x-8">
          ...
          {assetTo.isLocked && <Icon name="Lock" className="text-sys-color-text-primary" />}
        </div>
      </div>
    </div>
  );
