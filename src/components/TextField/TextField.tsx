  ...
  return (
    <div className="flex w-full flex-col gap-y-4">
      <div className="flex items-center justify-between">
        <span className="body-body1-medium text-sys-color-text-secondary">{label}</span>
        {showBalance && (
          <div className="gap-x-apollo-6 flex items-center">
            <Icon name="WalletSmall" size={12} className="text-sys-color-text-mute" />
            <div className="flex items-center gap-x-4">
              <span className="body-body1-medium text-sys-color-text-primary">
                {balanceValue}{' '}
                {balanceAsset && <span className="text-sys-color-text-mute">{balanceAsset}</span>}
              </span>
              {showLockIcon && <Icon name="Locks" size={14} className="text-sys-color-text-mute" />}
            </div>
          </div>
        )}
      </div>
      ...
      >
        ...
        <div className="flex w-full flex-col">
          ...
          {type === 'amount' && (
            <>
              <Divider />
              <div className={classNames('py-apollo-6 pr-apollo-6 flex items-center pl-16')}>
                {!invalid ? (
