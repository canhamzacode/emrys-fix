...
const PortfolioTransactionsHistorical = ({ solanaPubkey }: { solanaPubkey: PublicKey | null }) => {
  ...
  const [selectedTransaction, setSelectedTransaction] = useState<Interaction | null>(null);
  const [selectedTransactionConfirmations, setSelectedTransactionConfirmations] = useState<
    number[]
  >([]);

  const isDeposit = selectedTransaction?.interaction_type === InteractionType.Deposit;
    ...
    try {
      ...
      if (isDeposit) {
        ...
        const depositToHotReserveTxId = interactionDetail?.steps?.find(
          (step) => step.chain === Chain.Bitcoin && step.action === 'DepositToHotReserve',
        )?.transaction;
        ...
        if (depositToHotReserveTxId) {
          ...
          depositToHotReserveTxConfirmations = depositToHotReserveTxDetail?.confirmations ?? 0;
        }

        const lockToColdReserveTxId = interactionDetail?.steps?.find(
          (step) => step.chain === Chain.Bitcoin && step.action === 'LockToColdReserve',
        )?.transaction;
        ...
        if (lockToColdReserveTxId) {
          ...
          lockToColdReserveTxConfirmations = lockToColdReserveTxDetail?.confirmations ?? 0;
        }
    ...
    <div className="flex flex-col gap-y-24">
      ...
      <div className="block">
        <Table
          ...
          headers={['Date', 'Type', 'Status', 'Bitcoin Account', 'From', 'To', 'Action']}
          headerSize="large"
        >
          <TableHeader />
          <TableBody>
            ...
            {transactions && transactions.length === 0 ? (
              <div className="bg-sys-color-background-card rounded-16 gradient-border border-apollo-border-15 mt-8 flex flex-col items-center justify-center gap-y-24 py-64 md:mt-0 md:border-none">
                <div className="flex flex-col items-center justify-center gap-y-16 text-center">
                  <div className="headline-headline5 text-sys-color-text-primary">
                    {selectedTypes.length > 0 ? 'No Result Found' : 'No Completed Request Yet'}
                  </div>
              ...
              transactions.map((item) => (
                <TableRow size="large" key={item.interaction_id}>
                  <TableCell value={formatDate(item.initiated_at * 1000)}></TableCell>
                  <TableCell>
                    {item.interaction_type === InteractionType.Deposit ? 'Deposit' : 'Withdrawal'}
                  </TableCell>
                  <TableCell className="md:!py-0">
                    <span className={classNames('text-sys-color-text-mute flex md:hidden')}>
                    ...
                    rightIconOnClick={() => {
                      handleCopy(
                        item.interaction_type === InteractionType.Deposit
                          ? xOnlyPubkeyHexToP2tr(item.source, bitcoinNetwork, 'internal')
                          : xOnlyPubkeyHexToP2tr(item.destination, bitcoinNetwork, 'tweaked'),
                      );
                    }}
                  >
                    {item.interaction_type === InteractionType.Deposit
                  ...
                  <TableCell
                    leftIcon={item.interaction_type === InteractionType.Deposit ? 'btc' : 'zbtc'}
                    rightIcon={
                      item.interaction_type === InteractionType.Withdrawal && !item.is_stored
                        ? 'Lock'
                        : undefined
                    }
                    rightIconClassName="text-sys-color-text-mute"
                  >
                    {formatValue(new BigNumber(item.amount).dividedBy(10 ** BTC_DECIMALS), 6)}{' '}
                    <span className="text-sys-color-text-mute">
                      {item.interaction_type === InteractionType.Deposit ? 'BTC' : 'ZBTC'}
                    </span>
                  </TableCell>
                  <TableCell
                    leftIcon={item.interaction_type === InteractionType.Deposit ? 'zbtc' : 'btc'}
                    rightIcon={
                      item.interaction_type === InteractionType.Deposit ? 'Lock' : undefined
                    }
                    rightIconClassName="text-sys-color-text-mute"
                  >
                    {formatValue(
                    ...
                    <span className="text-sys-color-text-mute">
                      {item.interaction_type === InteractionType.Deposit ? 'zBTC' : 'BTC'}
                    </span>
                  </TableCell>
      ...
      <TransactionDetailsModal
        ...
        statusItems={
          isDeposit
            ...
            : getWithdrawalStatusItems(selectedTransaction, selectedTransactionConfirmations?.[0])
        }
        assetFrom={
          isDeposit
