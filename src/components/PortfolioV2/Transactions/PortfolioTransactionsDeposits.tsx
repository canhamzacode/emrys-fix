...
const PortfolioTransactionsDeposits = ({ solanaPubkey }: { solanaPubkey: PublicKey | null }) => {
    ...
    <div className="relative flex flex-col gap-y-24">
      <div className="block w-full">
        <Table
          ...
          headers={['Date', 'Status', 'Bitcoin Account', 'Locked', 'Minted', 'Action']}
          headerSize="large"
          showDividerOnMobile
              ...
              combinedTransactions.map((item) => (
                <TableRow size="large" key={item.interaction_id}>
                  ...
                  <TableCell className="md:!py-0">
                    <span
                      className={classNames(
                        ...
                        getInteractionStatusDisplay(item.status) === 'Pending' &&
                          '!text-apollo-brand-primary-blue',
                      )}
                    >
                      {item.deposit_block ? getInteractionStatusDisplay(item.status) : 'Initiating'}
                    </span>
                    <Chip
                      size="large"
                      label={
                  ...
                  <TableCell
                    ...
                    rightIconOnClick={() => {
                      handleCopy(xOnlyPubkeyHexToP2tr(item.source, bitcoinNetwork, 'internal'));
                    }}
                  >
                    {shortenString(
