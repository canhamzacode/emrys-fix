...
const PortfolioTransactionsWithdrawals = ({ solanaPubkey }: { solanaPubkey: PublicKey | null }) => {
  ...
  const [selectedTransactionConfirmations, setSelectedTransactionConfirmations] = useState(0);
    ...
    <div className="relative flex flex-col gap-y-24">
      <div className="block">
        <Table
          ...
          headers={['Date', 'Status', 'Bitcoin Account', 'Burned', 'Unlocked', 'Action']}
          headerSize="large"
        >
          <TableHeader />
          <TableBody>
              ...
              transactions?.map((item) => (
                <TableRow size="large" key={item.interaction_id}>
                  ...
                  <TableCell
                    ...
                    rightIconOnClick={() =>
                      handleCopy(xOnlyPubkeyHexToP2tr(item.destination, bitcoinNetwork, 'tweaked'))
                    }
                  >
                    {shortenString(
                  ...
                  <TableCell leftIcon="btc" rightIconClassName="text-sys-color-text-mute">
                    {formatValue(
