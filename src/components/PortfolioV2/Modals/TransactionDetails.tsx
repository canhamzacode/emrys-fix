        ...
        <div className="flex w-full flex-col items-center gap-y-24">
          ...
          <div className="flex w-full items-center justify-between">
            <div className="flex items-center gap-x-12">
              <Icon name="Transaction" size={18} className="text-apollo-brand-primary-blue" />
          ...
          >
            {tableItems.map((item, index) => (
              <Fragment key={item.label.label}>
                <TableRow size="medium/16px">
                  <TableCell
                    ...
                    rightIconClassName={cn(
                      item.label.rightIcon === 'InfoSmall' && 'cursor-pointer text-shade-mute',
                    )}
                  ...
                  >
                    ...
                    <span className="body-body1-medium">{item.label.label}</span>
                    {item.label.caption && (
                  ...
                  <TableCell
                    ...
                    rightIconClassName={cn('hover:!text-sys-color-text-primary')}
                    rightIconOnClick={
                ...
                {index !== tableItems.length - 1 && <Divider className="sm:!px-8" />}
              </Fragment>
            ))}
          </Table>
        )}
    ...
    >
      <>
        <div className="flex w-full flex-row justify-between gap-y-8 sm:w-auto sm:flex-col sm:justify-start">
          {type === 'deposit' && (
            <span className="body-body2-medium text-sys-color-text-secondary">Locked</span>
          )}
          {type === 'withdraw' && (
            <span className="body-body2-medium text-sys-color-text-secondary">Burned</span>
          )}
          ...
          <div className="flex items-center gap-x-8">
            ...
            {assetFrom.isLocked && <Icon name="Locks" className="text-sys-color-text-secondary" />}
          </div>
        </div>
        ...
        {assetTo && (
          <div className="flex w-full flex-row items-center justify-between gap-y-8 sm:w-auto sm:flex-col sm:items-start sm:justify-start">
            {type === 'deposit' && (
              <span className="body-body2-medium text-sys-color-text-secondary">Minted</span>
            )}
            {type === 'withdraw' && (
              <span className="body-body2-medium text-sys-color-text-secondary">Unlocked</span>
            )}
            <div className="flex items-center gap-x-8">
              ...
              {assetTo.isLocked && <Icon name="Locks" className="text-sys-color-text-secondary" />}
            </div>
          </div>
        )}
      </>
