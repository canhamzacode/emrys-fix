...
export default function RedeemModal({
  ...
  const handleChange = (e: React.ChangeEvent<HTMLInputElement>, decimals: number) => {
  ...
  const handleRedeem = async () => {
    ...
    try {
      ...
      const redeemAmountBN = new BN(
        new BigNumber(redeemAmount).multipliedBy(new BigNumber(10).pow(BTC_DECIMALS)).toString(),
      );
      ...
      for (const position of sortedPositions) {
        ...
        if (process.env.NEXT_PUBLIC_DEVNET_REDEEM_ADDRESS) {
          const targetAddress = new PublicKey(process.env.NEXT_PUBLIC_DEVNET_REDEEM_ADDRESS);
      ...
      <div className="flex w-full flex-col gap-y-24">
        ...
        <div className="flex w-full items-center justify-between">
          <div className="flex items-center gap-x-12">
            <Icon name="Withdraw01" size={18} className="text-apollo-brand-primary-blue" />
            <span className="body-body1-medium text-sys-color-text-secondary">Redeem zBTC</span>
          </div>
        </div>
        ...
        <div className="z-10 flex w-full items-center">
          <TextField
            ...
            value={redeemAmount}
            secondaryValue={`~$${
              Number(redeemAmount) > 0 ? formatValue(Number(redeemAmount) * btcPrice, 2) : 0
            }`}
              ...
              <div className="flex h-full w-full items-center space-x-8">
                ...
                <div className="flex flex-col">
                  <div className="flex items-center space-x-4">
                    <p className="headline-headline6 text-sys-color-text-primary">zBTC</p>
                    <Icon className="text-sys-color-text-secondary" name="Lock" size={14} />
                  </div>
        ...
        <div className="flex flex-col gap-y-8">
          <div className="flex items-center justify-between">
            <span className="body-body1-medium text-sys-color-text-secondary">Available In</span>
            <span className="body-body1-medium text-sys-color-text-secondary">~ 1 minute</span>
          </div>

          <div className="flex items-center justify-between">
            <span className="body-body1-medium text-sys-color-text-secondary">Remaining zBTC</span>
            <div className="body-body1-medium text-sys-color-text-secondary flex items-center gap-x-4">
              ...
              <Icon name="Lock" size={18} className="text-sys-color-text-secondary" />
            </div>
          </div>
        </div>
